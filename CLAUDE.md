...
When the CV is replaced, re-sync the places metrics actually live: `STATS` in `StatsBar.jsx`, the timeline entries in `Experience.jsx`, `SUPPORTING` + `BLURBS` in `Projects.jsx`, the prose and `FACTS` in `About.jsx`, and the hero paragraph in `Hero.jsx` plus the meta description in `index.html`. Read the repo READMEs too, not just the CV — the CV compresses figures for space (see the Northstar note above), and the demos quote the uncompressed versions. `DESIGN_NOTES.md` also carries a `### TODO: verify (illustrative / assumed values)` list to re-check at the same time.

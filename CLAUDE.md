...
**Experience holds employment only** (Koru Green → HCLTech → HDFC Life), and Education is its own section. An earlier build folded self-directed projects and both degrees into that one timeline and carried a disclosure note on the projects entry, because an unlabelled portfolio entry inside a work timeline reads as a client engagement. That entry is gone — it duplicated the three flagships in Projects — so the note went with it: the risk it guarded against no longer exists. **Do not re-add either.** Keep the timeline to paid roles; new independent work belongs in Projects. The word "unpaid" is not used anywhere on the site.
